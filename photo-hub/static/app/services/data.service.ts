...
export class DataService {
    ...
    uploadPhotoToAlbumJSON(image: any, title: string, albumId: number): Observable<void>{
        let json = JSON.stringify({image: image, title: title, albumId: albumId});
        console.log(json);
        return this.http.post(this.apiUri + 'photos/', json, this.headers())
            .map((res: Response) => {
                return res.json();
            })
            .catch(this.handleError);
